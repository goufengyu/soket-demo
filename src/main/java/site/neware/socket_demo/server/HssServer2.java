...
public  class HssServer2 extends Thread{
    ServerSocket server = null;
    Socket socket = null;
    public HssServer2(int port) {
        try {
    ...
    public void run(){
        ...
        try{
            ...
            System.out.println(socket.getInetAddress().getHostAddress() + ": 3000 " + "SUCCESS TO CONNECT...");
            InputStream in = socket.getInputStream();
            int len = 0;
            byte[] buf = new byte[1024];
            while ((len=in.read(buf))!=-1){
                System.out.println("（3000）接收到客户端消息: "+new String(buf,0,len));
            }
    ...
    public static void main(String[] args) {
        HssServer2 server = new HssServer2(3000);
        server.start();
    }
}
